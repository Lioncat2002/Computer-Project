...
#Functions
def TileMapRenderer(isFalling,isCollidingRight,isCollidingLeft):
    ...
    return isFalling,isCollidingRight,isCollidingLeft

def InputManager(movingRight,movingLeft,isJumping):
    ...
    if event.type==pygame.KEYUP:#Check if any key is released
        ...
        if event.key==pygame.K_LEFT:
            movingLeft=False
    return movingRight, movingLeft,isJumping


def Movement(playerPos,movingRight,movingLeft,isJumping):
        global jumpCount
...
while running:
    ...
    for event in pygame.event.get():#Getting the events
        if event.type == pygame.QUIT:#Checking if user has clicked X button on the window or not
            ...
            running = False
    #FunctionCall area
    isFalling, isCollidingRight, isCollidingLeft = TileMapRenderer(isFalling, isCollidingRight, isCollidingLeft)#Check collisions and draw tilemap

    movingRight, movingLeft, isJumping = InputManager(movingRight, movingLeft, isJumping)#Handle Input

    playerPos=Movement(playerPos, movingRight, movingLeft, isJumping)#Set player movement
    # FunctionCall area End

    screen.blit(playerSprite,playerRect)#Draw player sprite
